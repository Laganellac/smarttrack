...
# TODO: As a user, I want to be able to run a command like "smarttrack comment TEST_PROJECT Worked on new smarttrack features"
# which will add a comment to the currently active session of the given charge number. These comments should show up in the
# report generated by the "smarttrack report" command.
# def comment(args):
#     conn = get_connection()
#     c = conn.cursor()
#     c.execute('''
#         INSERT INTO comments (session_id, comment)
#         VALUES (?, ?)
#         ON CONFLICT(charge_number) DO UPDATE SET hourly_rate=excluded.hourly_rate
#     ''', (args.charge_number, args.hourly_rate))
